...
def assert_ready_for_upsampling(x, c, cin_pad):
    # 计算实际的上采样比例（upsample_scales 的乘积）
    upsample_ratio = np.prod(hparams.upsample_params.get('upsample_scales', [4, 4, 4, 4]))
    expected_len = (len(c) - 2 * cin_pad) * upsample_ratio
    # 严格对齐：不允许任何误差
    if len(x) != expected_len:
        print(f"Warning: Length mismatch - Expected x length {expected_len}, got {len(x)}. c length: {len(c)}, cin_pad: {cin_pad}, upsample_ratio: {upsample_ratio}")
        # 自动对齐：调整x的长度到期望值
        if len(x) > expected_len:
            x = x[:expected_len]
        else:
            pad_len = expected_len - len(x)
            x = np.pad(x, (0, pad_len), mode="constant")
        print(f"Auto-aligned: x length adjusted to {len(x)}")
    return x
...
def collate_fn(batch):
    ...
    if local_conditioning:
        ...
        for idx in range(len(batch)):
            ...
            if hparams.upsample_conditional_features:
                x = assert_ready_for_upsampling(x, c, cin_pad=0)
                if max_time_steps is not None:
                    ...
                    if len(x) > max_steps:
                        ...
                        x = assert_ready_for_upsampling(x, c, cin_pad=cin_pad)
            else:
    ...
    # 强制对齐条件长度到波形长度（时间维），避免后续模型中维度不匹配
    if c_batch is not None:
        x_T = x_batch.size(-1)
        c_T = c_batch.size(-1)
        if c_T != x_T:
            if c_T <= 0:
                raise RuntimeError("Invalid conditional length: 0")
            # 以整数重复为主，再做截断/补零
            repeat = max(1, x_T // max(1, c_T))
            c_batch = c_batch.repeat_interleave(repeat, dim=-1)
            if c_batch.size(-1) > x_T:
                c_batch = c_batch[:, :, :x_T]
            elif c_batch.size(-1) < x_T:
                pad_len = x_T - c_batch.size(-1)
                c_batch = torch.nn.functional.pad(c_batch, (0, pad_len))
...
def get_data_loaders(dump_root, speaker_id, test_shuffle=True):
    ...
    for phase in ["train_no_dev", "dev"]:
        ...
        if local_conditioning:
            Mel = FileSourceDataset(
                MelSpecDataSource(join(dump_root, phase), speaker_id=speaker_id,
                                  ...
                                  hop_size=audio.get_hop_size()))
            # 若该阶段没有收集到文件，跳过该阶段
            if len(X) == 0 or len(Mel) == 0:
                print("[{}]: no files collected, skip this phase".format(phase))
                continue
        ...
        if train:
            ...
            # 如果未提供 lengths（无 train.txt 情况），从已加载的数据计算长度
            if lengths.size == 0:
                if local_conditioning and Mel is not None:
                    lengths = np.array([Mel[i].shape[0] for i in range(len(Mel))])
                else:
                    lengths = np.array([X[i].shape[0] for i in range(len(X))])
            # Prepare sampler
            ...
            assert (sampler_idx == np.arange(len(sampler_idx), dtype=int)).all()
        else:
            sampler = None
...
if __name__ == "__main__":
    ...
    if dump_root is None:
        # 优先使用上级目录的 dump（与预处理产物一致），否则回退到示例数据路径
        candidate = join(dirname(__file__), "..", "dump")
        if exists(candidate):
            dump_root = os.path.abspath(candidate)
        else:
    ...
    if log_event_path is None:
        ...
        log_event_path = os.path.join("log", f"run-test-{ts}")
