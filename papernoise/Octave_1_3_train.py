...
# 导入数据集和模型
from noisedata import Octave_1_3_data  # 假设数据集类在noisedata.py中
from soundmodel import Octave_1_3_Model  # 假设多分枝模型在model.py中
...
def train_epoch(model, dataloader, criterion, optimizer, device, epoch, writer):
    ...
    for batch_idx, (inputs, targets, type_ids, mode_types) in enumerate(pbar):
        # print(targets)
        # 数据移至设备
        inputs = inputs.to(device)
        targets = targets.to(device)
        type_ids = type_ids.to(device)
        mode_types = mode_types.to(device)
        ...
        outputs = outputs[batch_size, mode_types, type_ids, :].squeeze()
