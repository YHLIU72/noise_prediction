...
#     def __init__(self, in_nc=7, nc=400, bowl_nc=2, mode_nc=4, num_bins=2501):
#         super(SPLModel, self).__init__()
#         self.num_bins = num_bins
#         self.mode_nc = mode_nc
#         self.bowl_nc = bowl_nc
...
#         self.out = nn.Linear(nc, bowl_nc * mode_nc * num_bins)
...
#         x = self.dropout3(x)
#         out = self.out(x)
#         output = out.view(-1, self.bowl_nc, self.mode_nc, self.num_bins)
#         return output
    #总声压级模型
class SPLModel(nn.Module):
    def __init__(self, in_nc=3, nc=400, mode_nc=4,type_nc=13, num_bins=1):
        super(SPLModel, self).__init__()
        self.num_bins = num_bins
        self.mode_nc = mode_nc
        ...
        self.dropout3 = nn.Dropout(0.2)
        self.relu = nn.LeakyReLU()
        self.out = nn.Linear(nc, mode_nc * type_nc * num_bins)
    def forward(self, inp):
        x = self.relu(self.hidden1(inp))
        x = self.dropout1(x)
        x = self.relu(self.hidden3(x))
        ...
        x = self.dropout3(x)
        out = self.out(x)
        output = out.view(-1, self.mode_nc, self.type_nc, self.num_bins)
        return output
    #1/3声压级模型
class Octave_1_3_Model(nn.Module):
    def __init__(self, in_nc=3, nc=400, mode_nc=4, type_nc=13, num_bins=28):
...
class LineOctaveModel(nn.Module):
    def __init__(self, in_nc=3, nc=400, mode_nc=4, type_nc=13, num_bins=2501):
