...
# 导入数据集和模型
from noisedata import LineOctavedata  # 假设数据集类在noisedata.py中
from soundmodel import LineOctaveModel  # 假设多分枝模型在model.py中
...
class TrainingConfig:
    def __init__(self):
        self.batch_size = 16
        self.epochs = 100
        self.model_name = "LineOctave"
        self.nc=16000
        self.learning_rate = 1e-5
        self.weight_decay = 1e-5
        self.val_split = 0.2
        self.random_seed = 42
        self.data_dir = "../csvdata222"  # 数据集路径
        self.log_dir = f"runs/line/exp_nc{self.nc}_epochs{self.epochs}_batchsize{self.batch_size}_lr{self.learning_rate}_wd{self.weight_decay}" + time.strftime("%Y%m%d_%H%M%S")
...
class HybridLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        # 计算总声压级（添加极小值避免log(0)）
        pred_total_lp = 10.0 * torch.log10(torch.sum(pred_lin, dim=1) + 1e-10)
        target_total_lp = 10.0 * torch.log10(torch.sum(target_lin, dim=1) + 1e-10)
...
def train_epoch(model, dataloader, criterion, optimizer, device, epoch, writer):
    ...
    for batch_idx, (inputs, targets, type_ids, mode_ids) in enumerate(pbar):
        # 数据移至设备
        inputs = inputs.to(device)
        targets = targets[:,:2501].to(device)
        ...
        outputs = outputs[batch_size, mode_ids, type_ids, :].squeeze(1)
        
        # 计算损失
        loss, mse, cosine, lp_loss = criterion(outputs, targets)
        # print(outputs[:3])
        # print(targets[:3])
        # print(loss.item(), mse.item(), cosine.item(), lp_loss.item())
...
def main(config):
    ...
    for epoch in range(config.epochs):
        ...
        if val_loss_avg < best_val_loss:
            ...
            checkpoint_path = os.path.join(config.checkpoint_dir, f"line_best_model_epoch.pth")
            torch.save({
                "epoch": epoch+1,
