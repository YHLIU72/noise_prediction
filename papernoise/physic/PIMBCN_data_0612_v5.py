"""
PIMBCN 数据集加载类（2026-06-12 V5: 增强数据+缓解过拟合）
...
[P0] 数据增强大幅增强：
  ...
  - 频谱噪声: 0.8→1.5 dB (1.9×)
  - 幅值缩放: ±8%→±15%
  - 频率偏移: [-3,4]→[-8,8] (范围扩大2.7×)
  - 新增 MixUp 频谱混合增强
[P0] 验证集分层: 确保每种工况≥2个验证样本
"""
...
class PIMBCNDataset(Dataset):
    ...
    def _split_data(self):
        ...
        for idx_c, n_val in zip(combo_indices, val_counts):
            if len(idx_c) <= 2:
                train_idx_list.extend(idx_c)  # 样本太少时不设验证集
            else:
    ...
    def _augment_inputs(self, inputs):
        # [V5] 噪声增强 0.02→0.05 (2.5×)
        return inputs + np.random.randn(*inputs.shape) * 0.05
    ...
    def _augment_spectrum(self, spectrum):
        ...
        # [V5] 高斯噪声 0.8→1.5 dB (1.9×)
        spectrum = spectrum + np.random.randn(len(spectrum)) * 1.5

        # [V5] 幅值缩放 ±8%→±15%
        spectrum = spectrum * (1.0 + np.random.uniform(-0.15, 0.15))

        # [V5] 频率偏移 [-3,4]→[-8,8] (范围扩大2.7×)
    ...
    def _mixup_spectrum(self, spectrum, idx):
        """[V5新增] MixUp: 与同工况随机样本线性混合"""
        # 查找同 mode+type 的另一个随机样本
