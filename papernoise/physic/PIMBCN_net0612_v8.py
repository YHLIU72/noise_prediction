"""
PIMBCN 网络模型（2026-06-12 V8: 高正则化共享Head版）

=== 设计理念 ===
V4 级正则化 (Dropout 0.4~0.5) + V5 架构增强 (FreqPE/embed=32/ch=128) + V4 损失 (5:2:3)
= 保留 V7-fix 的中位数优势(0.54) + 修复长尾(8.39→<6.0)

=== 相对 V7 的变更 ===
[修复] Dropout 全面提升: 0.2/0.15→0.4/0.3 (V4=0.5, V7=0.2, V8折中)
  - Encoder: 0.2/0.2/0.15 → 0.4/0.4/0.3
  - fc_expand: 0.3 → 0.5
  - refine: Dropout1d 0.2 → 0.3
  - freq_proj: 0.2 → 0.3
[修复] 损失权重: 5:10:3 → 5:2:3 (回归V4, LinearPeak=10误导小样本类型)
[保留] V5架构: FreqPE, embed_dim=32, channels=128, refine=48, SE reduction=2
[移除] DropPath (V6已证明有害)
"""
import torch
...
class FourierFeatureEmbedding(nn.Module):
    def __init__(self, input_dim=3, mapping_size=32, scale=1.0, hidden_dim=256):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(64, 128),  nn.LayerNorm(128), nn.GELU(), nn.Dropout(0.4),   # [V8] 0.2→0.4
            nn.Linear(128, 256), nn.LayerNorm(256), nn.GELU(), nn.Dropout(0.4),   # [V8] 0.2→0.4
            nn.Linear(256, hidden_dim), nn.LayerNorm(hidden_dim), nn.GELU(), nn.Dropout(0.3),  # [V8] 0.15→0.3
        )
...
# ============================================================================
# 共享解码器主干 — [V8] fc_expand Dropout 0.3→0.5, 其余同V7
# ============================================================================
class SharedDecoderBody(nn.Module):
    def __init__(self, input_dim=320, channels_init=128, seq_len_init=16):
        ...
        self.fc_expand = nn.Sequential(
            ...
            nn.GELU(), nn.Dropout(0.5),   # [V8] 0.3→0.5 (同V4)
        )
...
class SharedTaskHead(nn.Module):
    def __init__(self, freq_bins=1246, in_channels=16):
        ...
        self.refine = nn.Sequential(
            ...
            nn.LayerNorm(1024), nn.GELU(), nn.Dropout1d(0.3),   # [V8] 0.2→0.3
            nn.Conv1d(48, in_channels, kernel_size=3, padding=1),
        )
        self.freq_proj = nn.Sequential(
            nn.Linear(1024, 512), nn.LayerNorm(512), nn.GELU(), nn.Dropout(0.3),  # [V8] 0.2→0.3
...
# ============================================================================
# 物理损失 — [V8] 5:2:3 (回归V4, MSE主导 + 适度线性峰值)
# ============================================================================
class PhysicsLossWrapper(nn.Module):
    def __init__(self):
        super().__init__()
        self.weight_mse = 5.0         # [V8] V4权重
        self.weight_linear = 2.0      # [V8] V4权重 (V7=10 → 回归2)
        self.weight_grad = 3.0        # [V8] V4权重
        self.ms_w2 = 0.3
        self.ms_w4 = 0.1
