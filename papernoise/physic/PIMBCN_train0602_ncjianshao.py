...
# 请确保模型文件名为 PIMBCN_net0602_ncjianshao.py (即小样本高鲁棒版)
from PIMBCN_net0602_ncjianshao import PI_MBCN, PhysicsLossWrapper
...
def train_model(resume_path=None):
    ...
    head_learning_rate = 1e-5    # 大幅降低，任务头是线性层，梯度更敏感
    shared_learning_rate = 1e-4  # 适度降低
    freq_bins = 2501
    ...
    # 固定损失权重配置: MSE(7.39) > Sobolev(2.72) > Cosine(1) = OASPL(1)
    print(f"使用固定损失权重: MSE={loss_wrapper.weight_mse}, Cosine={loss_wrapper.weight_cosine}, \
          OASPL={loss_wrapper.weight_oaspl}, Gradient={loss_wrapper.weight_grad}")

    # ================= 优化器与调度器 =================
    # 分层权重衰减：共享部分使用更强正则化，任务头使用较弱正则化
    param_groups = [
        {'params': model.shared_encoder.parameters(), 'lr': shared_learning_rate, 'weight_decay': weight_decay_val},
        {'params': model.shared_decoder_body.parameters(), 'lr': shared_learning_rate, 'weight_decay': weight_decay_val},
        {'params': model.heads.parameters(), 'lr': head_learning_rate, 'weight_decay': weight_decay_val * 0.5},  # 任务头使用减半的权重衰减
    ]
    ...
    for epoch in epoch_bar:
        ...
        with torch.no_grad():
            ...
            for i in range(0, num_val_samples, batch_size):
                ...
                if amp_enabled:
                    with torch.amp.autocast('cuda'):
                        pred_spectrum = model(inputs, modes, types)
                        val_loss, val_mse, val_cos, val_oaspl, val_grad = loss_wrapper(pred_spectrum, target_spectrum)
                else:
                    pred_spectrum = model(inputs, modes, types)
                    val_loss, val_mse, val_cos, val_oaspl, val_grad = loss_wrapper(pred_spectrum, target_spectrum)

                # 修复：验证损失独立累加，不与训练损失混淆
        ...
        avg_train_mse = (epoch_train_mse / num_batches).item()
        avg_train_cosine = (epoch_train_cosine / num_batches).item()
        avg_train_oaspl = (epoch_train_oaspl / num_batches).item()
        avg_train_grad = (epoch_train_grad / num_batches).item()
        avg_val = (epoch_val_loss / num_val_batches).item()
        avg_val_mse = (epoch_val_mse / num_val_batches).item()
        avg_val_cosine = (epoch_val_cosine / num_val_batches).item()
        avg_val_oaspl = (epoch_val_oaspl / num_val_batches).item()
        avg_val_grad = (epoch_val_grad / num_val_batches).item()
