"""
PIMBCN 网络模型（2026-07-13 V15: 混合频率采样 — 低频对数+高频线性）

=== 基于 V4 架构，配合混合频率采样数据 ===
  核心思想: 模型结构不变, 变化仅在数据端 (混合频率重采样)。
  20~500Hz 对数采样 (保留V13低频优势), 500~5000Hz 线性采样 (恢复V4中频精度)。

=== 相对 V4/V13 的变更 ===
[无] 模型结构与 V4 完全一致
  - 数据侧使用混合频率重采样 (见 PIMBCN_data_0713_v15.py)
...
if __name__ == "__main__":
    ...
    print(f"V15 参数总量: {total:,}, 可训练: {trainable:,}")
    print("注意: V15 模型结构与 V4 完全相同, 变化仅在数据端 (混合频率重采样)")
