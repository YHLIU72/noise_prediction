...
# 确保导入更新后的模型和新的动态损失模块
from PIMBCN_net514 import PI_MBCN, PhysicsLossWrapper 
...
def train_model():
    ...
    # 定义基础学习率
    head_learning_rate = 5e-4   # 任务头和动态损失权重的学习率
    shared_learning_rate = 5e-4 # 共享主干的学习率
    freq_bins = 2501
    ...
    # ================= 核心修改 2：解耦学习率设置（包含损失参数） =================
    # 为共享模块、独立任务头以及动态损失权重分配不同的优化步长
    param_groups = [
        ...
        {'params': loss_wrapper.parameters(), 'lr': head_learning_rate} # 加入可学习的损失权重
    ]
    ...
    for epoch in range(epochs):
        model.train()
        loss_wrapper.train() # 确保 loss_wrapper 也在 train 模式，虽然它只有 Parameter 没有 Dropout/BN
        ...
        # 动态记录自适应损失权重 (转换回原始权重标度: exp(-log_var))
        with torch.no_grad():
        ...
        if (epoch + 1) % 10 == 0:
            print(f"Epoch [{epoch+1}/{epochs}] | Train Loss: {avg_train:.4f} | Val MSE: {avg_val:.4f} | LR: {current_lr_head:.2e}")
        
        # 最佳模型持久化
        if avg_val < best_val_loss:
            best_val_loss = avg_val
            # 注意：如果后续推理需要使用原汁原味的 MSE 或需要动态权重信息，
            # 最好把 loss_wrapper 的状态也保存下来，这里仅保存模型主体供推理使用
            torch.save({
