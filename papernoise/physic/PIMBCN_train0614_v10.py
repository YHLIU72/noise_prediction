"""
PIMBCN 训练脚本（2026-06-14 V10: 60~5000Hz 单共享Head架构）

=== 基于 V4 训练策略，频率范围调整为 60~5000Hz ===
  - freq_bins: 1246 → 1236 (截去 20~60Hz 高误差低频段, 方案A)
  - 模型结构、超参数、训练策略与 V4 完全一致

=== V4 训练策略（全部保留）===
[架构] 1共享Head, EMA+warmup+T_0=150+梯度累积
[损失] 5:2:3 (多分辨率MSE : LinearPeak : Sobolev)
...
from PIMBCN_net0614_v10 import PI_MBCN, PhysicsLossWrapper
from PIMBCN_data_0614_v10 import PIMBCNDataset
...
def train_model(resume_path=None):
    ...
    freq_bins = 1236          # [V10] 60~5000Hz, 1236点 (间隔4Hz)
    ...
    if resume_path is not None and os.path.exists(resume_path):
        ...
        run_name = f'pi_mbcn_v10_{timestamp}'
        save_dir = f'runs/{run_name}/models'
        os.makedirs(save_dir, exist_ok=True)
        writer = SummaryWriter(f'runs/{run_name}')
        print(f"从头训练 (V10: 1共享Head, 60~5000Hz), 时间戳: {timestamp}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='PI-MBCN V10 训练脚本 (60~5000Hz)')
