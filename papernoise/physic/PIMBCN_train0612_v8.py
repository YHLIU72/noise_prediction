"""
PIMBCN 训练脚本（2026-06-12 V8: 高正则化共享Head版）

=== 设计理念 ===
V4级正则化 + V5架构 + 5:2:3损失 = V7-fix中位数优势 + V4长尾鲁棒性
  - 数据: V4级弱增强 (噪声0.02/频谱0.8dB/频移[-3,4])
  - 架构: V5增强 (embed=32, ch=128, FreqPE, refine=48)
  - 正则化: Dropout 0.4~0.5 (V4=0.5, V7=0.2, V8折中)
  - 损失: 5:2:3 (回归V4, MSE主导)
  - 训练: wd=1e-3, T_0=150, patience=2000, EMA+warmup
...
from PIMBCN_net0612_v8 import PI_MBCN, PhysicsLossWrapper
from PIMBCN_data_0612_v8 import PIMBCNDataset
...
def train_model(resume_path=None, stage2_from=None):
    ...
    if resume_path is not None and os.path.exists(resume_path):
        ...
        run_name = f'pi_mbcn_v8_{timestamp}'
        save_dir = f'runs/{run_name}/models'
        os.makedirs(save_dir, exist_ok=True)
        writer = SummaryWriter(f'runs/{run_name}')
        print(f"从头训练 (V8: 高正则化), 时间戳: {timestamp}")
    ...
    ema_decay = 0.999
    print(f"损失权重 (V8 5:2:3): MSE={loss_wrapper.weight_mse}, "
          f"LinearPeak={loss_wrapper.weight_linear}, Sobolev={loss_wrapper.weight_grad}")
    print(f"正则化: Dropout 0.4~0.5, wd={weight_decay_val}, 无DropPath")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='PI-MBCN V8 训练脚本 (高正则化共享Head)')
