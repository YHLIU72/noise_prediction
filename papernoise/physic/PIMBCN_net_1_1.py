...
class PI_MBCN(nn.Module):
    ...
    def forward(self, x, mode_idx, type_idx):
        ...
        for i in range(batch_size):
            ...
            h_i = hidden[i:i+1]
            s_pred, dynamic_alpha = self.branches[branch_key](h_i)
            
            spectrum_out[i] = s_pred[0]
            alpha_out[i] = dynamic_alpha.squeeze()
            
        return spectrum_out, alpha_out

def physics_informed_loss_fn(pred_spectrum, learned_alpha, target_spectrum, inputs, rpm_mean, rpm_std, lambda_phy=1000000):
    """
    增强版物理信息损失函数
    移除OASPL和倍频程预测头后的简化版本
    """
    ...
    # 计算频谱能量作为OASPL的代理，用于物理约束
    # OASPL = 10 * log10(sum(10^(spectrum/10))) ≈ 10 * log10(sum(exp(spectrum * ln(10) / 10)))
    pred_oaspl = 10.0 * torch.log10(torch.sum(torch.pow(10.0, pred_spectrum / 10.0), dim=1, keepdim=True))
    ...
    total_loss = loss_mse_spec + 200.0 * loss_cosine_spec + lambda_phy * loss_physics
    
    return total_loss, loss_mse_spec, loss_cosine_spec, loss_physics