...
from PIMBCN_net_1_1 import PI_MBCN, physics_informed_loss_fn
...
def train_model():
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (inputs, types, modes, _, _, target_spectrum) in enumerate(train_bar):
            ...
            pred_spectrum, alpha = model(inputs, modes, types)
            batch_alphas.append(alpha.mean().item()) 
            
            # 【应用动态物理权重】
            loss, loss_mse_spec, loss_cosine_spec, loss_physics = physics_informed_loss_fn(
                pred_spectrum, alpha, 
                target_spectrum, 
        ...
        with torch.no_grad():
            for inputs, types, modes, _, _, target_spectrum in val_bar:
                ...
                pred_spectrum, _ = model(inputs, modes, types)
