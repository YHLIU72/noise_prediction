"""
PIMBCN 数据集加载类（2026-06-14 V13 配套版: 对数频率重采样）

=== 相对 V4 的变更 ===
[V13] 对数频率重采样:
  - 从原始 2501 点线性频谱 (0~10000Hz, 4Hz间隔) 中截取 20~5000Hz
  - 使用 logspace 在 20~5000Hz 范围内取 1246 个采样点
  - 低频段 (20-100Hz): ~150 个采样点 (vs V4 的 ~20 个)
  - 高频段 (2000-5000Hz): ~400 个采样点 (vs V4 的 ~750 个)
  - 目的: 在总频点数不变的前提下, 给低频段更多"注意力分辨率"

[保留] V4 全部数据管道: 分层抽样、预解析numpy、弱增强
"""
...
class PIMBCNDataset(Dataset):
    def __init__(self, directory_path, input_cols=[4, 5, 6],
        ...
        # [V13] 对数频率轴: logspace(20, 5000, 1246)
        self.freq_axis = np.logspace(np.log10(20), np.log10(5000), 1246)
        # 原始线性频率轴用于插值
    ...
    def _preparse_all_columns(self):
        ...
        for val in self.data.iloc[:, self.spectrum_col]:
            ...
            # 先截取 V4 原始线性频谱: 20~5000Hz (索引 5~1250)
            linear_spectrum = np.array(parsed[5:1251], dtype=np.float32)  # [1246]
            # 对数插值: 从线性频率轴映射到对数频率轴
            log_spectrum = np.interp(self.freq_axis, self._orig_linear_freqs, linear_spectrum)
            spectrum_list.append(log_spectrum.astype(np.float32))
        ...
        print(f"预解析完成: inputs {self.inputs_array.shape}, "
              f"spectrum {self.spectrum_array.shape} (20~5000Hz, 对数采样)")
        print(f"[V13] 对数频率采样: 低频(<100Hz) {low_count}点, "
              f"中频(100-1000Hz) {np.sum((self.freq_axis>=100)&(self.freq_axis<1000))}点, "
              f"高频(>1000Hz) {np.sum(self.freq_axis>=1000)}点")
    ...
    def _augment_spectrum(self, spectrum):
        ...
        # [V13] 对数域频移: 缩小范围, 避免跨越对数尺度不匹配
        shift = np.random.randint(-2, 3)  # V4: [-3,4] → V13: [-2,3]
        if shift != 0:
