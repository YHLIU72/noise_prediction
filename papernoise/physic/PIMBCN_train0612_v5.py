"""
PIMBCN 训练脚本（2026-06-12 V5: 增强正则化+两阶段训练）

=== 相对 V4 的变更 ===
[P0] 损失权重重校准: MSE=1.0, LinearPeak=300, Grad=1.0 (三者贡献均衡)
[P1] weight_decay: 1e-3→5e-3 (5×, 强力缓解过拟合)
[P2] T_0: 150→500 (更长的余弦周期, 充分探索)
[P3] 早停机制: patience=500, 防止无效训练
[P3] 两阶段训练 (可选): --stage2 从头训练Stage1(冻结主干)后再Stage2全模型微调
[保留] V4训练策略: EMA+warmup+num_workers=2+梯度累积+混合精度

超参数: lr=3e-4, wd=5e-3, batch=8, accum=2(等效16), epochs=50000
"""
import os
...
from PIMBCN_net0612_v5 import PI_MBCN, PhysicsLossWrapper
from PIMBCN_data_0612_v5 import PIMBCNDataset
...
def train_model(resume_path=None, stage2_from=None):
    ...
    freq_bins = 1246
    weight_decay_val = 5e-3                                          # [V5] 1e-3→5e-3
    ...
    if resume_path is not None and os.path.exists(resume_path):
        ...
        run_name = f'pi_mbcn_v5_{timestamp}'
        save_dir = f'runs/{run_name}/models'
        os.makedirs(save_dir, exist_ok=True)
        writer = SummaryWriter(f'runs/{run_name}')
        print(f"从头训练 (V5: 增强正则化), 时间戳: {timestamp}")
    ...
    print(f"预期贡献比例 ~ MSE:Grad:Peak = 0.56:0.52:0.24")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='PI-MBCN V5 训练脚本')
