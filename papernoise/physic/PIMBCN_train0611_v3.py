"""
PIMBCN 训练脚本（2026-06-11 V3: 梯度累积 + 嵌入注入 + 多分辨率损失）

=== 相对 0610_final 的增量变更 ===
[策略1] 梯度累积: accum_steps=2, 等效 batch=16
[策略2] 多分辨率损失 → 模型侧实现
[策略3] Mode/Type 嵌入 → 模型侧实现
[保留] EMA + LR warmup + T_0=150 + num_workers=2
...
from PIMBCN_net0611_v3 import PI_MBCN, PhysicsLossWrapper
from PIMBCN_data_0611_v3 import PIMBCNDataset
...
def train_model(resume_path=None):
    ...
    # ===================== 超参数 =====================
    batch_size = 8
    accum_steps = 2           # 梯度累积, 等效 batch=16
    epochs = 50000
    ...
    if resume_path is not None and os.path.exists(resume_path):
        ...
        run_name = f'pi_mbcn_v3_{timestamp}'
        save_dir = f'runs/{run_name}/models'
        os.makedirs(save_dir, exist_ok=True)
        writer = SummaryWriter(f'runs/{run_name}')
        print(f"从头训练 (V3: 嵌入+多分辨率+梯度累积), 时间戳: {timestamp}")
    ...
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True,
                              ...
                              drop_last=True)  # 梯度累积需要固定 batch 数
    ...
    for epoch in epoch_bar:
        ...
        for batch_idx, (inputs, types, modes, _, _, target_spectrum) in enumerate(train_loader):
            ...
            if amp_enabled:
                ...
                # 梯度累积: 除以 accum_steps 保持等效 lr 不变
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='PI-MBCN V3 训练脚本')
