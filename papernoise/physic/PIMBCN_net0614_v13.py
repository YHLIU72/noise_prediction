"""
PIMBCN 网络模型（2026-06-14 V13: 方案3 — 对数频率重采样）

=== 基于 V4 架构，配合对数频率采样数据 ===
  核心思想: 不改变模型结构，而是在数据预处理阶段用对数频率坐标重新采样频谱。
  低频段(20-100Hz)采样点从 ~20 增加到 ~150, 高频段(2000-5000Hz)从 ~750 减少到 ~400。
  模型在总频点数不变(1246)的前提下，获得对低频段更高的"有效分辨率"。

=== 相对 V4 的变更 ===
[无] 模型结构与 V4 完全一致
  - 数据侧已在对数域重采样 (见 PIMBCN_data_0614_v13.py)
  - freq_bins 仍为 1246, 模型无需感知频率尺度变化
...
if __name__ == "__main__":
    ...
    print(f"V13 参数总量: {total:,}, 可训练: {trainable:,}")
    print("注意: V13 模型结构与 V4 完全相同, 变化仅在数据端 (对数频率重采样)")
