"""
PIMBCN 训练脚本（2026-06-12 V4: 单共享Head架构）

=== 相对 V0611_v3 的变更 ===
[架构] 52独立Head → 1共享Head, param_groups中用 shared_head 替代 heads
[保留] 所有V0611_v3训练策略: EMA+warmup+T_0=150+num_workers=2+梯度累积
...
def train_model(resume_path=None):
    ...
    if resume_path is not None and os.path.exists(resume_path):
        ...
        run_name = f'pi_mbcn_v4_{timestamp}'
        save_dir = f'runs/{run_name}/models'
        os.makedirs(save_dir, exist_ok=True)
        writer = SummaryWriter(f'runs/{run_name}')
        print(f"从头训练 (V4: 1共享Head), 时间戳: {timestamp}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='PI-MBCN V4 训练脚本')
