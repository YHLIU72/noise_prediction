"""
PIMBCN 训练脚本（2026-06-12 V7-fix v2: 修复续训bug + patience=2000）

=== 相对 V7 的变更 ===
[修复] early_stop_patience: 500→2000
  - V7在ep1247达到最优后被patience=500截杀
  - V4从ep1247→ep3556持续改善了2309轮
  - V7在ep1247时val_mse_ms=2.10 (优于同期V4的2.25!), 但没机会走完V4的路
[修复] 续训 global_step 从 start_epoch 推算 (v1 bug: 固定为0 → warmup错误重放)
[修复] 续训 best_epoch 从 checkpoint 恢复 (v1 bug: 丢失原最优epoch信息)
[修复] 续训 ema 初始化为已训练权重 (v1 bug: ema从随机权重开始,验证结果不准)
[保留] V7所有设计: V4级弱增强 + V5架构 + 5:10:3损失 + wd=1e-3 + T_0=150
"""
import os
...
def train_model(resume_path=None, stage2_from=None):
    ...
    if resume_path is not None and os.path.exists(resume_path):
        ...
        best_epoch = checkpoint.get('epoch', -1)             # [fix] 恢复原最优epoch
        ...
        run_name = f'pi_mbcn_v7fix_{timestamp}'
        save_dir = f'runs/{run_name}/models'
        os.makedirs(save_dir, exist_ok=True)
        writer = SummaryWriter(f'runs/{run_name}')
        print(f"从头训练 (V7-fix: patience=2000), 时间戳: {timestamp}")
    ...
    if is_resuming:
        # [fix] 续训时 ema 初始化为已训练的模型权重, 而非随机权重
    ...
    # [fix] 续训: global_step 从已完成步数开始, 自然跳过 warmup (原bug: 固定为0)
    global_step = start_epoch * len(train_loader) if is_resuming else 0
    ...
    # ===================== 早停机制 — [V7-fix] patience=2000 =====================
    early_stop_patience = 2000
    early_stop_counter = 0
    print(f"早停patience: {early_stop_patience} (V7-fix: 500→2000, 适应余弦周期震荡)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='PI-MBCN V7-fix 训练脚本 (patience=2000)')
