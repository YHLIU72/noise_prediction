"""
PIMBCN 训练脚本（2026-06-12 V7: 回归基线增强版）

=== 设计理念 ===
V4弱增强 + V5架构 + 5:10:3损失 = 吸取V4泛化力 + V5表达能力
  - 数据: V4级弱增强 (噪声0.02/频谱0.8dB/频移[-3,4])
  - 架构: V5增强 (embed_dim=32, channels=128, FreqPE, refine=48)
  - 损失: 5:10:3 (MSE主导, LinearPeak适度10x)
  - 无DropPath, wd=1e-3, T_0=150
...
def train_model(resume_path=None, stage2_from=None):
    ...
    if resume_path is not None and os.path.exists(resume_path):
        ...
        run_name = f'pi_mbcn_v7_{timestamp}'
        save_dir = f'runs/{run_name}/models'
        os.makedirs(save_dir, exist_ok=True)
        writer = SummaryWriter(f'runs/{run_name}')
        print(f"从头训练 (V7: 回归基线增强), 时间戳: {timestamp}")
    ...
    scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=150, T_mult=2, eta_min=1e-7)  # [V7] 恢复V4 T_0
    ...
    # ===================== 早停机制 =====================
    early_stop_patience = 500                                     # [V7] 恢复合理值
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='PI-MBCN V7 训练脚本')
