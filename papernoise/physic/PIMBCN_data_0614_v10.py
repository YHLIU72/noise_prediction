"""
PIMBCN 数据集加载类（2026-06-14 V10 配套版）
基于 V4 架构，频率范围调整为 60~5000Hz（方案A：截去 20~60Hz 高误差低频段）。

=== 相对 V4 的变更 ===
[频率范围] 20~5000Hz (1246点) → 60~5000Hz (1236点, 间隔4Hz)
  - 原始 2501 点频谱 (0~10000Hz) 截取索引: [5:1251] → [15:1251]
  - 理由: 20~60Hz 验证集 RMSE ~2.8dB，是 100Hz+ 的 2.2 倍，截去提升整体精度
  - freq_bins: 1246 → 1236

[保留] V4 全部数据管道设计: 分层抽样、预解析numpy、弱增强(噪声0.02/频谱0.8dB)
"""
...
class PIMBCNDataset(Dataset):
    def __init__(self, directory_path, input_cols=[4, 5, 6],
        ...
        self.freq_axis = np.linspace(60, 5000, 1236)  # [V10] 60~5000Hz, 1236点
    ...
    def _preparse_all_columns(self):
        ...
        for val in self.data.iloc[:, self.spectrum_col]:
            ...
            spectrum_list.append(parsed[15:1251])  # [V10] 60~5000Hz (索引15=60Hz, 1250=5000Hz)
