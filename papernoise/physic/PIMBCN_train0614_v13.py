"""
PIMBCN 训练脚本（2026-06-14 V13: 方案3 — 对数频率重采样）

=== 基于 V4 训练策略，配合对数频率采样数据 ===
[V13] 数据预处理阶段使用对数频率重采样
  - 低频段(20-100Hz): ~150 采样点 (V4的 7.5×)
  - 高频段(2000-5000Hz): ~400 采样点 (V4的 53%)
  - 模型结构与 V4 完全一致, 仅数据端变化

=== V4 训练策略（全部保留）===
[架构] 1共享Head, EMA+warmup+T_0=150+梯度累积
[增强] 弱增强 (噪声0.02/频谱0.8dB/频移[-2,3] 对数域缩小范围)
...
from PIMBCN_net0614_v13 import PI_MBCN, PhysicsLossWrapper
...
def train_model(resume_path=None):
    ...
    freq_bins = 1246          # 20~5000Hz, 对数采样 1246 点
    ...
    if resume_path is not None and os.path.exists(resume_path):
        ...
        run_name = f'pi_mbcn_v13_{timestamp}'
        save_dir = f'runs/{run_name}/models'
        os.makedirs(save_dir, exist_ok=True)
        writer = SummaryWriter(f'runs/{run_name}')
        print(f"从头训练 (V13: 对数频率重采样, 20~5000Hz), 时间戳: {timestamp}")
    ...
    print(f"频率范围: 20~5000Hz (对数采样), 频点数: {freq_bins}")
    print(f"梯度累积: {accum_steps}步, 等效batch={batch_size * accum_steps}")
    print(f"[V13] 对数频率采样已启用: 低频密度 7.5×, 高频密度 0.53× (vs V4线性)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='PI-MBCN V13 训练脚本 (对数频率重采样)')
