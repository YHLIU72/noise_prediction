"""
PIMBCN 训练脚本（2026-06-14 V14: 方案3 — 修复V10增强 + 损失重校准 + 延长warmup）

=== 基于 V10 训练策略，三处关键修复 ===
[V14修复1] 数据增强: 频移np.roll→np.pad(reflect), 消除60Hz边界伪影
[V14修复2] 损失权重: MSE 5→6, LinearPeak 2→3, Sobolev 3→1.5
[V14修复3] warmup: 5→20 epoch, 给freq_proj层更多时间稳定学习映射

[保留] V4/V10训练策略: EMA+warmup+T_0=150+梯度累积+弱增强
...
from PIMBCN_net0614_v14 import PI_MBCN, PhysicsLossWrapper
from PIMBCN_data_0614_v14 import PIMBCNDataset
...
def train_model(resume_path=None):
    ...
    freq_bins = 1236          # [V14] 60~5000Hz, 1236点 (间隔4Hz)
    ...
    if resume_path is not None and os.path.exists(resume_path):
        ...
        run_name = f'pi_mbcn_v14_{timestamp}'
        save_dir = f'runs/{run_name}/models'
        os.makedirs(save_dir, exist_ok=True)
        writer = SummaryWriter(f'runs/{run_name}')
        print(f"从头训练 (V14: 方案3 修复增强+重校准, 60~5000Hz), 时间戳: {timestamp}")
    ...
    print(f"频率范围: 60~5000Hz, 频点数: {freq_bins}")
    print(f"梯度累积: {accum_steps}步, 等效batch={batch_size * accum_steps}")
    print(f"[V14] 增强修复: 频移反射填充 | 损失: MSE=6.0, Peak=3.0, Grad=1.5 | warmup=20ep")
    ...
    # ===================== [V14] LR warmup 延长: 5→20 epoch =====================
    warmup_epochs = 20; warmup_steps = warmup_epochs * len(train_loader)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='PI-MBCN V14 训练脚本 (修复V10增强+重校准)')
