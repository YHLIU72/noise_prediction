"""
PIMBCN 网络模型（2026-06-14 V10: 60~5000Hz 单共享Head架构）

=== 基于 V4 架构，频率范围调整为 60~5000Hz ===
  - freq_bins: 1246 → 1236 (截去 20~60Hz 高误差低频段)
  - 模型结构、损失权重、注意力机制与 V4 完全一致
  - 仅 adapt 输出维度以匹配新频率范围
...
[微调] 卷积通道小幅增强
  ...
  - broadband: 8→6→1
  - tonal_peak: 8→6→1
  - freq_proj: 1024→512→freq_bins

[保留] 嵌入注入+多分辨率损失+IN+EMA+warmup+梯度累积
"""
import torch
...
# ============================================================================
# 共享任务头 (唯一, freq_bins=1236 适配 60~5000Hz)
# ============================================================================
class SharedTaskHead(nn.Module):
    def __init__(self, freq_bins=1236):  # [V10] 1236 点 (60~5000Hz, 间隔4Hz)
        super().__init__()
        self.freq_bins = freq_bins
        self.refine = nn.Sequential(
            nn.Conv1d(8, 24, kernel_size=3, padding=1),   # 8→24
...
class PI_MBCN(nn.Module):
    def __init__(self, num_modes=4, num_types=13, freq_bins=1236, embed_dim=16):  # [V10] freq_bins=1236
        super().__init__()
    ...
    def forward(self, x, mode_idx, type_idx):
        ...
        return self.shared_head(features)                            # [B, 1236] (60~5000Hz)


# ============================================================================
# 物理损失 V3: 多分辨率MSE + 线性峰值 + Sobolev (与 V4 完全一致)
# ============================================================================
