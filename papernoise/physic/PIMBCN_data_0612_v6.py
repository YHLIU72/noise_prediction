"""
PIMBCN 数据集加载类（2026-06-12 V6: 折中增强 — 平衡拟合与泛化）

=== 相对 V5 的变更 ===
[修正] 数据增强回调至折中强度 (V5过强导致欠拟合):
  - 输入噪声: 0.05→0.03 (V4=0.02, V5=0.05, V6=0.03 ← 折中)
  - 频谱噪声: 1.5→1.0 dB (V4=0.8, V5=1.5, V6=1.0 ← 折中)
  - 频率偏移: [-8,8]→[-6,6] (V4=[-3,4], V5=[-8,8])
  - MixUp 概率: 30%→15% (降低异常样本风险)
  - 幅值缩放: ±15%→±12% (V4=±8%, V5=±15%)
[保留] V5验证集min-2分层
"""
...
class PIMBCNDataset(Dataset):
    ...
    def _split_data(self):
        ...
        for size in combo_sizes:
            count = max(2, int(np.round(size * self.val_split)))
            count = min(count, max(1, size // 2))
            val_counts.append(count)
        total_assigned = sum(val_counts)
        if total_assigned > target_val:
            ratio = target_val / total_assigned
            val_counts = [max(2 if size >= 4 else 1, int(np.round(c * ratio))) for size, c in
                          zip(combo_sizes, val_counts)]
            val_counts = [min(c, max(1, size // 2)) for size, c in zip(combo_sizes, val_counts)]
        for idx_c, n_val in zip(combo_indices, val_counts):
            if len(idx_c) <= 2:
                train_idx_list.extend(idx_c)
            else:
        ...
        print(f"实际验证集比例: {len(self.val_indices)/total_samples:.2%} (目标: {self.val_split:.2%})")
    ...
    def _augment_inputs(self, inputs):
        # [V6] 折中: V4=0.02, V5=0.05, V6=0.03
        return inputs + np.random.randn(*inputs.shape) * 0.03
    ...
    def _augment_spectrum(self, spectrum):
        ...
        # [V6] 折中: V4=0.8, V5=1.5, V6=1.0
        spectrum = spectrum + np.random.randn(len(spectrum)) * 1.0

        # [V6] 折中: V4=±8%, V5=±15%, V6=±12%
        spectrum = spectrum * (1.0 + np.random.uniform(-0.12, 0.12))

        # [V6] 折中: V4=[-3,4], V5=[-8,8], V6=[-6,6]
    ...
    def _mixup_spectrum(self, spectrum, idx):
        """[V5新增, V6保留] MixUp: 与同工况随机样本线性混合"""
        combo = self.modes_array[idx] * 1000 + self.types_array[idx]
        same_combo_idx = np.where(
            (self.modes_array == self.modes_array[idx]) &
            (self.types_array == self.types_array[idx])
        )[0]
        if len(same_combo_idx) < 2:
            return spectrum
        other = np.random.choice(same_combo_idx[same_combo_idx != idx])
        other_spec = self.spectrum_array[other].copy()
        lam = np.random.beta(0.4, 0.4)
        lam = max(lam, 1 - lam)
        ref_self = np.max(spectrum)
        ref_other = np.max(other_spec)
        other_spec_aligned = other_spec - ref_other + ref_self
        return lam * spectrum + (1 - lam) * other_spec_aligned
    ...
    def __getitem__(self, idx):
        ...
        if self.augment and not self.is_validation:
            ...
            if np.random.random() < 0.15:
                spectrum = self._mixup_spectrum(spectrum, data_idx)
