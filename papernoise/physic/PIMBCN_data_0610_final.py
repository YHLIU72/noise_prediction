"""
PIMBCN 数据集加载类（2026-06-10 最终版）

=== 相对原始版的变更 ===
1. 频谱范围: 0~10kHz(2501点)→20~5kHz(1246点), 切片索引5~1250
2. OASPL约束自洽: 增强时基于20~5kHz频谱自算OASPL, 而非全频段CSV值
3. 验证集共享 numpy 数组: get_validation_dataset 不重复加载CSV
4. 分层抽样: 按 mode×type 52种工况组合均匀划分训练/验证集
"""
...
class PIMBCNDataset(Dataset):
    ...
    def _load_and_process_data(self):
        ...
        self.data[type_col_name] = self.type_encoder.fit_transform(self.data[type_col_name])
        self.data[mode_col_name] = self.mode_encoder.fit_transform(self.data[mode_col_name])
        print(f"成功加载 {len(self.data)} 条数据记录")

    # ---------- 预解析到 numpy (频谱截取20~5kHz) ----------
    def _preparse_all_columns(self):
        ...
        for val in self.data.iloc[:, self.spectrum_col]:
            ...
            parsed = parsed[5:1251]  # 20~5000Hz @4Hz, 1246点
    ...
    def _augment_spectrum(self, spectrum):
        ...
        # 记录增强前 OASPL (基于20~5kHz范围)
        scaled = spectrum / 10.0
        max_v = np.max(scaled)
        ...
        if np.random.random() > 0.5:
            w = np.random.randint(3, 7)
            win = np.hanning(w); win = win / win.sum()
        ...
        spectrum = spectrum * (target_oaspl / new_oaspl)
        return spectrum
